...
impl SharingStore {
    pub fn filter_by_indices(&self, indices: &[i32]) -> Self {
        ...
        // 收集 ciphers 中匹配的元素
        let filtered_ciphers: Vec<(i32, DVector<R>, DVector<R>, Store)> = indices
            .iter()
            .filter_map(|&idx| {
                self.ciphers
                    .iter()
        ...
        // 收集 u_vec 中匹配的元素
        let filtered_u_vec: Vec<(i32, DVector<R>)> = indices
            .iter()
            .filter_map(|&idx| self.u_vec.iter().find(|(i, _)| *i == idx).cloned())
            .collect();

        let filtered_mfs: Vec<(i32,Proof<Hash>)> = indices.iter().filter_map(|&idx| {
...
pub fn supple_share(vss_st: SharingStore, pks: &Vec<(i32, PublicKey)>) -> Vec<SuppleShare> {
