...
use ve::r_ring::R;
use crate::components::{PrivateShare, PublicShare};
...
impl Serialize for PrivateShare {
    ...
    where
        S: Serializer,
    {
        let mut st = serializer.serialize_struct("PrivateShare", 6)?;
        ...
        A: MapAccess<'de>,
    {
        ...
        while let Some(key) = map.next_key::<String>()? {
            match key.as_str() {
                ...
                _ => return Err(A::Error::unknown_field(&key, &["id", "v", "w", "c", "z", "lemma", "path"])),
            }
        }
